...
# # vendor_excel_file_path = r'C:\Users\somep\OneDrive\Documents\Automation\Seafysoft\vendorEmails.xlsx'
...
#         <body>
#             <h3>Hello,</H3>
#             <p>At <strong>Safesoft Solutions</strong>, we take pride in our team of highly skilled consultants, who are ready to begin working on-site or remotely at your convenience. Whether you have immediate needs or future projects in mind, we are here to support you.</p>
#             <h1><span style="background-color: yellow; padding: 0 5px;">Reach us at: hr@seafysoft.com</span></h1>
#             <br/>
#             <p>{email_body}</p>
#             <p>Best regards,</p>
...
#     except Exception as e:
#         print(f"Failed to send email to {receiver_email}: {str(e)}")
#         return False, str(e)  # Return the error message for further handling
...
#             # Send email
#             email_status, error_message = send_email(sender_email, receiver_email, sender_password, email_body)
...
#             else:
#                 # Check if the error indicates hitting the daily limit
#                 if "Daily user sending limit exceeded" in error_message:
#                     print("Daily user sending limit exceeded. Switching to next email account.")
#                     # Switch email account and continue from current index
#                     sender_email, sender_password = get_account_details(email_sent_count, email_limit)
...
#                         email_sent_count += 1
#                     else:
#                         print(f"Failed to send email to {receiver_email} with new account. Error: {error_message}")
#                         break  # Stop if it still fails
...
#     except Exception as e:
#         print(f"Failed to send email to {receiver_email}: {str(e)}")
#         return False, str(e)  # Return success status and error message
...
#                 # If it's a tuple, it means there was an error
#                 email_status, error_message = email_status
#                 if "Daily user sending limit exceeded" in error_message:
#                     print("Daily user sending limit exceeded. Switching to next email account.")
#                     # Switch email account and continue from current index
#                     sender_email, sender_password = get_account_details(email_sent_count, email_limit)
...
#                     if email_status:  # Success on retry
#                         vendor_df.at[index, 'Email Sent Status'] = 'Sent'
#                         vendor_df.at[index, 'Email Sent Date'] = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
#                         email_sent_count += 1
#                         vendor_df.to_excel(vendor_excel_file_path, index=False)  # Save changes after sending
#                     else:
#                         print(f"Failed to send email to {receiver_email} with new account.")
#                         break  # Stop if it still fails
#             else:
#                 # Update the DataFrame immediately after sending the email
#                 vendor_df.at[index, 'Email Sent Status'] = 'Sent'
#                 vendor_df.at[index, 'Email Sent Date'] = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
#                 email_sent_count += 1
#                 vendor_df.to_excel(vendor_excel_file_path, index=False)  # Save changes after each email
#                 time.sleep(2)  # Small delay between sends
